 ...
 window.$Techdicer = window.$Techdicer || (function(){
    "use strict";
    return {
                        ...
                        for(var i = 0; i < tabs.length; i++){
                            ...
                            for(var j=0;j<cookiesList.length;j++){
                                ...
                                if(tab.url && tab.url.indexOf(cookie.domain) >= 0){
                                    ...
                                    cookie.isLex = window.$Techdicer.isLightningExperience(tab.url);
                            ...
                            for(var j=0;j<cookiesList.length;j++){
                                ...
                                if(!tmpCookiesMap[tmpCookie.oid] 
                                    || (tmpCookie.isActive
                                        && tmpCookie.isMaster)
                                    || (!window.$Techdicer.isLightningExperience(tmpCookie.domain)
                                        && tmpCookie.isMaster
                            ...
                            for(var key in tmpCookiesMap){
                                if(!tmpCookiesMap[key].isLex 
                                    && !window.$Techdicer.isLightningExperience(tmpCookiesMap[key].domain)) continue;

                                for(var j = 0; j < cookiesList.length; j++){
                                    if(cookiesList[j].oid !== key) continue;
                                    if(cookiesList[j].isLex
                                        || window.$Techdicer.isLightningExperience(cookiesList[j].domain)) continue;
        ...
        getSobjects: function(hostname, sessionId, callback){
            ...
            var _url = 'https://' + hostname + '/services/data/v' + window.$Techdicer.API_LEVEL + '/sobjects/';
            var _headers = {
        ...
        getSalesforceLimits: function(hostname, sessionId, callback){

            var _url = 'https://' + hostname + '/services/data/v' + window.$Techdicer.API_LEVEL + '/limits/';
            var _headers = {
                'Authorization':'Bearer ' + sessionId
            };
        ...
        getSalesforceQueryResult: function(hostname, sessionId, query, callback){

            var _url = 'https://' + hostname + '/services/data/v' + window.$Techdicer.API_LEVEL + '/query/?q=' + query;
            var _headers = {
                'Authorization':'Bearer ' + sessionId
            };
        ...
        getSalesforceRestResponse: function(hostname, sessionId, apiEndpoint, httpMethod, requestBody, callback){

            var _url = 'https://' + hostname + apiEndpoint;
            var _headers = {
                'Authorization':'Bearer ' + sessionId,
        ...
        startMetadataRetrieve: function( hostname, sessionId, metaTypes, callback) {
            ...
            const _url = 'https://' + hostname + '/services/Soap/m/' + window.$Techdicer.API_LEVEL;
            ...
            const _body = `<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:met="http://soap.sforce.com/2006/04/metadata">
                                <soapenv:Header>
                                <met:CallOptions>
                                    <met:client>${window.$Techdicer.CLIENT_ID}</met:client>
                                </met:CallOptions>
                                <met:SessionHeader>
                                ...
                                <met:retrieve>
                                    <met:retrieveRequest>
                                        <met:apiVersion>${window.$Techdicer.API_LEVEL}</met:apiVersion>
                                        <met:singlePackage>true</met:singlePackage>
                                        <met:unpackaged>
                                            <met:fullName>Techdicer</met:fullName>
                                            ...
                                            <met:version>${window.$Techdicer.API_LEVEL}</met:version>
                                        </met:unpackaged>
                                    </met:retrieveRequest>
                                </met:retrieve>
                                </soapenv:Body>
